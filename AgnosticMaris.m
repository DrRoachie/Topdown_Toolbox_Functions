function [] = AgnosticMaris(Animal, RecDate, Epoch, Condition_1_data, Condition_2_data, Shared_ChannelPairs, Frequency_Band, ...
                          savedir, RandomIteration, threshold_value)
...
% granger statistical tests running in parallel to save time. This
% specific version is meant to the process different version of congruent
% and non-congruent data.
...
freq_Condition_1      = ft_freqanalysis(cfg, Condition_1_data);
freq_Condition_2      = ft_freqanalysis(cfg, Condition_2_data);
...
Condition_1_Coh_Spectrum       = ft_connectivityanalysis(cfg, freq_Condition_1);
Condition_2_Coh_Spectrum       = ft_connectivityanalysis(cfg, freq_Condition_2 );
...
Granger_Condition_1      = ft_connectivityanalysis(cfg, freq_Condition_1 );
Granger_Condition_2      = ft_connectivityanalysis(cfg, freq_Condition_2);

Granger_Condition_1.dof   = length(Condition_1_data.trial);
Granger_Condition_2.dof   = length(Condition_2_data.trial);
...
% calculate z statistic for every channel pair
    ...
    Granger_data_Maris_zstatistic_AC_PFC    = [];
            ...
            for k = 1:length(Shared_ChannelPairs(:,1))
                ...
                % Use the coherence spectra of condition 1 and condition 2 to calculate the Maris z-statistic for the data 
                Coh_test_statistic_a = atanh(abs(Condition_1_Coh_Spectrum.cohspctrm(k,:))) - (1/(Condition_1_Coh_Spectrum.dof-2)); % derived from Maris et al 2007 Eq 1 
                Coh_test_statistic_b = atanh(abs(Condition_2_Coh_Spectrum.cohspctrm(k,:))) - (1/(Condition_2_Coh_Spectrum.dof-2));
                Coh_test_statistic_c = sqrt((1/(Condition_1_Coh_Spectrum.dof-2))+(1/(Condition_2_Coh_Spectrum.dof-2)));
                ...
                Send_Chan_idx   = find(contains(Granger_Condition_1.label, Send_Chan_lb));    % find the index of the sending/receiving channels to map onto granger spectrum
                Rec_Chan_idx    = find(contains(Granger_Condition_2.label, Rec_Chan_lb));     % Granger_PriorOnly.label and Granger_PretoneOnly.label are identical
                
                % PFC to AC
                    Granger_Vec_Condition_1   = squeeze(Granger_Condition_1.grangerspctrm(Send_Chan_idx, Rec_Chan_idx, :));
                    Granger_Vec_Condition_2   = squeeze(Granger_Condition_2.grangerspctrm(Send_Chan_idx, Rec_Chan_idx, :));
    
                    Granger_test_statistic_a = atanh(abs(Granger_Vec_Condition_1)) - (1/(Granger_Condition_1.dof-2)); % derived from Maris et al 2007 Eq 1 
                    Granger_test_statistic_b = atanh(abs(Granger_Vec_Condition_2)) - (1/(Granger_Condition_2.dof-2));
                    Granger_test_statistic_c = sqrt((1/(Granger_Condition_1.dof-2))+(1/(Granger_Condition_2.dof-2)));
                ...
                % AC to PFC
                    Granger_Vec_Condition_1   = squeeze(Granger_Condition_1.grangerspctrm(Rec_Chan_idx, Send_Chan_idx, :));
                    Granger_Vec_Condition_2 = squeeze(Granger_Condition_2.grangerspctrm(Rec_Chan_idx, Send_Chan_idx, :));
...
cat_Condition_1_c.label       = Condition_1_data(1).label;
cat_Condition_1_c.time        = [Condition_1_data(:).time];
cat_Condition_1_c.trial       = [Condition_1_data(:).trial];
cat_Condition_1_c.fsample     = Condition_1_data(1).fsample;
cat_Condition_1_c.sampleinfo  = vertcat(Condition_1_data.sampleinfo);

% concatonate Pretone data across all sessions 
cat_Condition_2_c.label      = Condition_2_data(1).label;
cat_Condition_2_c.time       = [Condition_2_data(:).time];
cat_Condition_2_c.trial      = [Condition_2_data(:).trial];
cat_Condition_2_c.fsample    = Condition_2_data(1).fsample;
cat_Condition_2_c.sampleinfo = vertcat(Condition_2_data.sampleinfo);

% Get trial number of pretone and prior conditions and set random partition size

Condition_1_c_TrialNum   =  length(cat_Condition_1_c.trial);
Condition_2_c_TrialNum =  length(cat_Condition_2_c.trial);
Total_c_TrialNum = Condition_1_c_TrialNum + Condition_2_c_TrialNum;
RandomPartition_1_Size_c = max(Condition_2_c_TrialNum, Condition_1_c_TrialNum);
...
All_Trials_c.label      = Condition_1_data(1).label;
All_Trials_c.fsample    = Condition_1_data(1).fsample;
All_Trials_c_struct     = [cat_Condition_2_c, cat_Condition_1_c];
                ...
                freq_2        = ft_freqanalysis(cfg, Random_Partition_2);
                    ...
                    for k = 1:length(Shared_ChannelPairs(:,1))
                        ...
                        % PFC to AC
                            ...
                            Granger_test_statistic_c = sqrt((1/(Granger_Condition_1.dof-2))+(1/(Granger_Condition_2.dof-2)));
                            Granger_test_statistic_Zavg = (Granger_test_statistic_b- Granger_test_statistic_a)/Granger_test_statistic_c;
                            Granger_Monte_Carlo_Array_Maris_PFC_AC(i,:,k) = Granger_test_statistic_Zavg'; % squeeze function makes granger vector change shape, so taking the transpose reshapes it to the proper dimensions
...
% calculate the p-value for correct trials 
    ...
    Fig1_name      = sprintf('%s_%s_%s_%s_%s_Coh_zthresh.fig', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
    Fig2_name      = sprintf('%s_%s_%s_%s_%s_Coh_montecarlo.fig', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
    save_file_name = sprintf('%s_%s_%s_%s_%s_Coh_data.mat', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
    ...
    pvalue_AC_PFC.all = 1 - pvalue_AC_PFC.all;
                ...
                Fig1_name      = sprintf('%s_%s_%s_%s_%s_Granger_zthresh_PFC_to_AC.fig', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
                Fig2_name      = sprintf('%s_%s_%s_%s_%s_Granger_montecarlo_PFC_to_AC.fig', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
                save_file_name = sprintf('%s_%s_%s_%s_%s_Granger_data_PFC_AC.mat', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
                ...
                save(fullfile(savedir, save_file_name), 'data_granger_Maris_Z_PFC_AC', 'data_z_statistic_Maris_threshold_PFC_AC', 'all_cluster_sums_Maris_PFC_AC', 'all_clusters_Maris_PFC_AC', ...
                                                        ...
                                                        'monte_max_cluster_sum_Maris_PFC_AC', 'pvalue_PFC_AC');     % appends to the granger data file that saves in the pairwise connectivity uber
                ...
                save_file_name = sprintf('%s_%s_%s_%s_%s_Granger_data_AC_PFC.mat', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);

                Fig1_name      = sprintf('%s_%s_%s_%s_%s_Granger_zthresh_AC_to_PFC.fig', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
                Fig2_name      = sprintf('%s_%s_%s_%s_%s_Granger_montecarlo_AC_to_PFC.fig', Animal, RecDate, Epoch, Current_ChanPair_lb, Frequency_Band);
                ...
                save(fullfile(savedir, save_file_name), 'data_granger_Maris_Z_AC_PFC', 'data_z_statistic_Maris_threshold_AC_PFC', 'all_cluster_sums_Maris_AC_PFC', 'all_clusters_Maris_AC_PFC', ...
                                                        ...
                                                        'monte_max_cluster_sum_Maris_AC_PFC', 'pvalue_AC_PFC');     % appends to the granger data file that saves in the pairwise connectivity uber
