...
function [shuffled_xcorr_result, shuffled_lagResults, xcorr_result, lagResults] = TrialbyTrialXcorr(datadir, Animal, RecDate, Epoch, Condition, Current_ChanPair, Behavior)
...
params.SNR = SNR;
        ...
        numTrial_w = length(data_w.trial);
        ...
        % Preallocate the matrix to store the extracted rows
        numCells = numel(data_w.trial);
        numTimePoints = size(data_w.trial{1}, 2); % Assuming each trial has the same number of time points
        PFC_extractedRows = zeros(numCells, numTimePoints);
        ...
        % Calculate the length of the cross-correlation result
        [shuffled_w, shuffled_lags] = xcorr(PFC_extractedRows_shuffled(1, :), AC_extractedRows_shuffled(1, :));
        shuffled_numLags = length(shuffled_w);
        ...
        for i = 1:numRows
            ...
            % Calculate the cross-correlation
            [shuffled_w, shuffled_lags] = xcorr(rowPFC_shuffled, rowAC_shuffled, 'coeff');  % The 'coeff' option in xcorr scales the cross-correlation so that the autocorrelation at zero lag 
                                                        % (i.e., the cross-correlation of the signal with itself) is equal to 1. 
                                                        % This is achieved by dividing the cross-correlation values by the product of 
                                                        % the standard deviations of the two signals.
            ...
            % Store the result in the preallocated arrays
            shuffled_xcorrResults(i, :) = shuffled_w;
        ...
        shuffled_xcorr_result.avg = mean(shuffled_xcorrResults, 1);
        shuffled_xcorr_result.sem = std(shuffled_xcorrResults, 0, 1) / sqrt(numTrial_w);
